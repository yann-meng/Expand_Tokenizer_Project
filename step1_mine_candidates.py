...
"""
Step 1:
从语料中挖候选词，导出 candidate_tokens.csv 供人工审核。

功能：
1. 读取单文件或目录中的文本
2. 自动跳过坏样本
3. 清洗语料
4. 训练 SentencePiece 辅助 tokenizer
5. 统计 SP 词片频次
6. 结合原 Qwen tokenizer 计算压缩收益
7. 导出可人工审核 CSV

依赖：
    pip install -U sentencepiece transformers

示例：
    python step1_mine_candidates.py \
      --base_model Qwen/Qwen3-1.7B-Base \
      --input_path ./my_corpus_dir \
...
def parse_args():
    ...
    parser.add_argument("--max_source_files_per_token", type=int, default=3)
...
def detect_readable_text_file(path: Path) -> bool:
    try:
        with open(path, "rb") as f:
            head = f.read(4096)
        return not is_binary_like(head)
    except Exception:
        return False
...
def prepare_clean_corpus(
    ...
    stats = {
        ...
        "num_binary_skipped_files": 0,
        "num_total_raw_lines": 0,
        "num_written_lines": 0,
        "num_empty_skipped": 0,
        "num_too_short_skipped": 0,
        "num_too_long_skipped": 0,
        "num_bad_text_skipped": 0,
        "num_sampled_out": 0,
        "num_dedup_skipped": 0,
    }
    ...
    with open(merged_corpus_path, "w", encoding="utf-8") as out_f:
        for idx, fp in enumerate(files, start=1):
            ...
            if idx % 100 == 0:
                print(f"  processed files: {idx}/{len(files)}")
...
def is_reasonable_token(piece: str, min_chars: int, max_chars: int) -> bool:
    if not piece:
        return False
    if len(piece) < min_chars or len(piece) > max_chars:
        return False
...
def count_candidates_and_examples(
    corpus_path: str,
    sp,
    base_tokenizer,
    min_freq: int,
    min_piece_chars: int,
    max_piece_chars: int,
    min_old_token_len: int,
    max_candidates: int,
    allow_whitespace_piece: bool,
    max_examples_per_token: int,
    max_source_files_per_token: int,
) -> List[Dict]:
    ...
    for piece, freq in freq_counter.most_common():
        ...
        candidates.append({
            "candidate": piece,
            "freq": int(freq),
            "sample_coverage": int(sample_cover_counter[piece]),
            "old_token_len": int(old_len),
            "score": float(score),
    ...
    candidates.sort(
        key=lambda x: (-x["score"], -x["freq"], -x["sample_coverage"], -x["old_token_len"], x["candidate"])
    )
    ...
    with open(path, "w", encoding="utf-8-sig", newline="") as f:
        ...
        for item in candidates:
            writer.writerow({
                ...
                "old_token_len": item["old_token_len"],
...
def main():
    ...
    corpus_stats = prepare_clean_corpus(
    ...
    sp_model_file, sp_vocab_file = train_sentencepiece(
        corpus_path=merged_corpus_path,
    ...
    candidates = count_candidates_and_examples(
        corpus_path=merged_corpus_path,
        sp=sp,
        base_tokenizer=base_tokenizer,
        min_freq=args.min_freq,
        min_piece_chars=args.min_piece_chars,
        max_piece_chars=args.max_piece_chars,
        min_old_token_len=args.min_old_token_len,
        max_candidates=args.max_candidates,
        allow_whitespace_piece=args.allow_whitespace_piece,
        max_examples_per_token=args.max_examples_per_token,
        max_source_files_per_token=args.max_source_files_per_token,
    )
    ...
    print(f"Clean corpus : {merged_corpus_path}")
